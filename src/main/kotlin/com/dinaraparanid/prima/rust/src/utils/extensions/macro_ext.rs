...
macro_rules! impl_track_traits {
    ($track_type:ty) => {
        impl crate::entities::tracks::track_trait::TrackTrait for $track_type {
            #[inline]
        ...
        impl PartialEq for $track_type {
            ...
            fn eq(&self, other: &Self) -> bool {
                self.path
                    .eq(crate::entities::tracks::track_trait::TrackTrait::get_path(
                        other,
                    ))
            }
        }
...
macro_rules! impl_artist_traits {
    ($artist_type:ty) => {
        impl crate::entities::artists::artist_trait::ArtistTrait for $artist_type {
            #[inline]
...
macro_rules! impl_playlist_traits {
    ($playlist_type:ident) => {
        impl<T: crate::utils::extensions::track_ext::TrackExt> IntoIterator for $playlist_type<T> {
            type Item = T;
        ...
        impl<Tr: crate::utils::extensions::track_ext::TrackExt> Extend<Tr> for $playlist_type<Tr> {
            #[inline]
            fn extend<T: IntoIterator<Item = Tr>>(&mut self, iter: T) {
                crate::entities::playlists::playlist_trait::PlaylistTrait::push_all(self, iter)
            }
        }

        impl<T: crate::utils::extensions::track_ext::TrackExt> From<$playlist_type<T>>
            for yaml_rust::Yaml
        {
            #[inline]
            fn from(playlist: $playlist_type<T>) -> Self {
                ...
                hash.insert(
                    ...
                    Yaml::Array(Array::from_iter(playlist.into_iter().map(|t| {
                        crate::utils::extensions::track_ext::TrackExt::to_yaml(&t)
                    }))),
                );
        ...
        impl<T: crate::utils::extensions::track_ext::TrackExt>
            crate::entities::playlists::playlist_trait::PlaylistTrait<T> for $playlist_type<T>
        {
            #[inline]
            fn get_title(&self) -> Option<&String> {
                self.title.as_ref()
            ...
            #[inline]
            fn get_type(&self) -> crate::entities::playlists::playlist_type::PlaylistType {
                self.tp
            }
