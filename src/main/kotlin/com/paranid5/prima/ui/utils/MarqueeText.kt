...
fun MarqueeText(
    ...
    LaunchedEffect(textLayoutInfoState.value) {
        ...
        do {
            ...
            do {
                ...
                offset.value = (animation.getValueFromNanos(playTime))
    ...
    SubcomposeLayout(modifier = modifier.clipToBounds()) { constraints ->
        ...
        var mainText = subcompose(MarqueeLayers.MainText) { createText(textModifier) }
            .first()
        ...
        when {
            mainText.width <= constraints.maxWidth -> {
                mainText = subcompose(MarqueeLayers.SecondaryText) { createText(textModifier.fillMaxWidth()) }
                    .first()
                    .measure(constraints)
                ...
                val secondTextOffset = mainText.width + offset.value + spacing
                val secondTextSpace = constraints.maxWidth - secondTextOffset

                if (secondTextSpace > 0)
                    secondPlaceableWithOffset = subcompose(MarqueeLayers.SecondaryText) { createText(textModifier) }
                        .first()
...
@Composable
private fun GradientEdge(startColor: Color, endColor: Color) = Box(
    modifier = Modifier
        .width(10.dp)
        .fillMaxHeight()
